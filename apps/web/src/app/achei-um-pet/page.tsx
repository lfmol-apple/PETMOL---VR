...
export default function AcheiUmPetPage() {
  ...
  const handleMarkFound = async (id: string) => {
    ...
    if (!token) {
      alert('Entre no PETMOL para marcar como encontrado.');
      return;
    }
    setMarkingFound(id);
    try {
      const res = await fetch(`${API_BASE_URL}/missing-pets/${id}/found`, {
        method: 'PATCH',
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-white/50 mt-1.5 leading-relaxed">
                Estes pets precisam de você. Se reconhecer algum, ligue para o tutor imediatamente.
                {activeCount > 0 && (
        ...
        {!isFound && (
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              disabled={marking}
              className="px-4 py-3 rounded-2xl bg-white/8 border border-white/10 text-white/60 text-[13px] font-bold active:scale-[0.98] transition-all disabled:opacity-40"
            >
              {marking ? '...' : '✓ Encontrei'}
            </button>
          </div>
