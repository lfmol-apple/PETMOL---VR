...
const [showVaccineSheet, setShowVaccineSheet] = useState(false);
    ...
    <div className="space-y-2">
      {visibleAlerts.map((alert) => (
        <MissingPetAlertCard
          ...
          collapsed={collapsedAlertIds.includes(alert.id)}
          onToggleCollapsed={(collapsed) => setAlertCollapsed(alert.id, collapsed)}
          onViewCard={() => setAlertCard(alert)}
          onSeeThis={() => router.push(`/achei-um-pet?id=${alert.id}`)}
          onDismiss={() => {
            writeDismissedId(alert.id);
            setHandledAlertIds(prev => [...new Set([...prev, alert.id])]);
            setNearbyAlerts(prev => prev.filter(a => a.id !== alert.id));
          }}
          onReport={() => setReportAlert(alert)}
        />
      ))}
  ...
  // visibleNearbyAlerts já foi calculado mais acima (antes do early-return
  // de boot) — aqui só o subtítulo do badge do botão "Pet Sumido".
  const nearbyMissingCount = visibleNearbyAlerts.length;
      ...
      {showNearbyNotice && (
        <NearbyMissingPetsNotice
          ...
          onOpen={() => {
            ...
            // Ver o carrossel também silencia a soneca pro mesmo conjunto de
            // alertas — quem já abriu e viu não precisa ser interrompido de
            // novo na próxima abertura da Home só por não ter "dispensado".
