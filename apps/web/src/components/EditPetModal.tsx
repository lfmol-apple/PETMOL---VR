...
export function EditPetModal({ pet, photoVersion, onClose, onSave, onDelete }: EditPetModalProps) {
  ...
  const hasPhoto = Boolean(formData.photo.trim()) || Boolean(petPhotoUrl);
  const canSubmit =
    formData.name.trim().length > 0 &&
    hasPhoto &&
    (!breedRequired || formData.breed.trim().length > 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasPhoto) { setError('Adicione uma foto do pet.'); return; }
    if (breedRequired && !formData.breed.trim()) { setError('Selecione a raça do pet (use "SRD" se não souber).'); return; }
    setLoading(true);
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain px-5 py-5 space-y-5">
              ...
              <div className="flex flex-col items-center gap-2">
                <button type="button" onClick={() => setShowPhotoPicker(true)}
                  className={`relative w-28 h-28 rounded-full bg-slate-50 border-2 border-dashed flex items-center justify-center active:scale-[0.97] transition-transform ${hasPhoto ? 'border-slate-300' : 'border-rose-300'}`}>
                  {petPhotoUrl ? (
                ...
                <button type="button" onClick={() => setShowPhotoPicker(true)}
                  ...
                  {hasPhoto ? 'Trocar foto' : 'Adicionar foto do pet *'}
                </button>
                {!hasPhoto && <span className="text-[11px] text-rose-500 font-semibold -mt-1">Obrigatória</span>}
              </div>
