// Gera o ícone NATIVO (iOS/Android) em 1024×1024 sem alpha, com a MESMA
// arte "glossy" dos ícones PWA (ver scripts/render-pwa-icons.cjs): fundo em
// gradiente vetorial + máscara branca "P + patinha" extraída de
// apps/web/public/brand/petmol-logo-official.png.
//
// Escreve apps/web/assets/icon.png — a fonte que o @capacitor/assets usa
// pra gerar o AppIcon do iOS/Android. Depois de rodar:
//   cd apps/web && npx @capacitor/assets generate --assetPath assets --ios
//
// Uso: node scripts/render-native-icon.cjs
...
const OUT = path.join(root, 'apps/web/assets/icon.png');
const SIZE = 1024;
...
async function main() {
  ...
  const image = await sharp(iconSvg(SIZE))
    ...
    .flatten({ background: '#02246F' })
    .removeAlpha()
    .png({ compressionLevel: 9 })
    .toBuffer();

  await fs.writeFile(OUT, image);
  console.log(`wrote ${path.relative(root, OUT)} (${SIZE}x${SIZE}, no alpha)`);
}
