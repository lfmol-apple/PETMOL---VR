"""
Contrato global anti-regressão pra toda a superfície de buy-path
pública (seção 4/5 da revisão de 25/08/2026, ver docs/BUY_PATH_AUDIT.md).

Objetivo explícito: NÃO reimplementar/duplicar os testes específicos
por merchant/endpoint que já existem (test_petz_integration.py,
test_awin_click_redirect.py, test_marketplace_offer_provider.py,
test_handoff_partner.py, test_handoff_shopping.py, etc.) — este arquivo
existe só pra pegar uma REGRESSÃO FUTURA: se alguém adicionar um
endpoint/CTA novo que esqueça de checar os gates comerciais, um destes
dois testes quebra.
"""
...
def _reset_settings(monkeypatch):
    for var in (
        "PETZ_AFFILIATE_ENABLED", "PETZ_COUPON_ATTRIBUTION_VERIFIED",
        "MERCADOLIVRE_AFFILIATE_ENABLED", "PETZ_AFFILIATE_URL",
        "COBASI_AFFILIATE_URL", "PETLOVE_DOG_LIFE_URL",
    ):
...
def test_no_unmonetized_public_buy_paths(client):
    """Estado padrão de produção (affiliate-only, nenhum merchant
    comercialmente provado ligado) — nenhum caminho de compra público
    devolve URL nenhuma."""
...
def test_affiliate_only_never_returns_direct_link(client, monkeypatch):
    """Mesmo com produto, oferta e direct_url REAIS cadastrados — quando
    affiliate-only está ligado, nenhuma resposta pública contém
    direct_url nem link_type="direct"."""
