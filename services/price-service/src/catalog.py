...
def _load_food_catalog_from_file(filename: str) -> List[CatalogProduct]:
    """Load a structured food database (brand/line/variant/weight, one entry
    per real SKU) from src/catalogs/food_database/<filename>.

    Used for both foods_br_phase1.json (AI-compiled, unverified — kept only
    as a historical artifact, no longer loaded by default) and
    foods_br_phase2_cobasi.json (real SKUs crawled live from Cobasi's public
    catalog via scripts/build_food_catalog_from_cobasi.py, each with a real
    EAN). Fase 1 was the source of essentially every wrong-brand/wrong-flavor
    bug found in production (Nattu, Dog Chow, missing pack-size variants) —
    it was generated from an LLM's "general knowledge", never checked against
    a real product; a confident wrong match from it was worse than no catalog
    match at all, since it overrode the AI's own (correct) reading of the
    photo. Fase 2 replaces it as the default-loaded file.
    """
    path = os.path.join(os.path.dirname(__file__), "catalogs", "food_database", filename)
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError) as exc:
        logger.warning("[catalog] food database %s not loaded: %s", filename, exc)
        return []
    ...
    for item in data.get("items", []):
        ...
        try:
            ...
            logger.warning("[catalog] skipping malformed item %r from %s: %s", item.get("id"), filename, exc)
            continue
    logger.info("[catalog] Loaded %d food products from %s into BR catalog", len(products), filename)
    return products
...
def search_catalog_candidates(
    ...
    for _, product in scored:
        ...
        # came from a real confirmed scan; "cobasi-{itemId}" entries came from
        # a live crawl of Cobasi's real catalog (real EAN, real listing) via
        # foods_br_phase2_cobasi.json — neither is hand-curated guesswork, so
        # both get tagged distinctly from plain catalog_static and trusted
        # more by the caller's scoring (see resolver.ts's source-trust bonus).
        # The old Fase 1 (ai_compiled, unverified) catalog is what caused
