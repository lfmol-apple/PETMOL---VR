...
Os três devem apontar pro mesmo `sha`. Compare com `git log -1 --format=%H main` localmente para confirmar que produção bate com o GitHub — qualquer divergência entre os três indica incidente de deploy (ver [`docs/RUNBOOK.md`](docs/RUNBOOK.md)).
...
- [`.github/workflows/deploy-atomic.yml`](.github/workflows/deploy-atomic.yml) é o **pipeline automático oficial**: dispara via `workflow_run` assim que o CI passa em `main`. Baixa o artefato já empacotado pelo CI, envia por SSH, extrai em `/opt/petmol/releases/<sha>/`, roda a bateria de health checks e só então ativa a release trocando o symlink `/opt/petmol/current` (ativação atômica). Se um health check essencial falhar, faz rollback automático pra release anterior — sem intervenção manual.
