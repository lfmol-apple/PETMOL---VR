...
| Shopee | **ativa — só vitrine** (shortlink afiliado). Ofertas por produto desligadas até o projeto de precisão (`docs/LAUNCH.md` §7) | `shopee_affiliate_enabled=False` (default); frontend `affiliateStatus: 'active'` (a vitrine é frontend, não depende do flag) |
...
| Shopee | Shopee Affiliates | `MarketplaceOffer`/`MarketplaceOfferProvider` (busca textual — API não casa por GTIN) | **só VITRINE no lançamento** — shortlink afiliado (`NEXT_PUBLIC_AFFILIATE_SHOPEE` / `DEFAULT_SHOPEE_AFFILIATE_URL`). Ofertas por produto DESLIGADAS (`shopee_affiliate_enabled=False` por padrão) até o projeto de precisão — ver `docs/LAUNCH.md` §7 | n/a | pré-requisitos cumpridos 21/08; per-produto pendente (sem lookup GTIN, sync acoplado à Awin off, ofertas defasadas) |
