...
| Consentimento de IA para fotos | `user_consents` (`provider=google_gemini`, `consent_type=ai_photo_processing`) | Enquanto a conta existir ou até revogação; removido no delete da conta |
| Fotos enviadas pra IA (Gemini) | Processadas via `/vision/*`; envio ao Gemini só acontece após consentimento explícito por usuário, consultado no backend (ver `vision/router.py` e `features/ai/aiPhotoConsent.ts`) | Foto original: mesma retenção do documento/produto que ela virou; tratamento no Gemini segue os termos/configurações do serviço Google contratado |
