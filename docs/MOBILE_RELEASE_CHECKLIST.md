...
- [x] Xcode instalado e build/archive funcionando — **resolvido** (o app já foi submetido à App Store em 07/09/2026, o que exige um archive real gerado localmente). Nota histórica original (24/08/2026, não reflete mais o estado atual): "Bloqueado hoje: Xcode completo não está instalado neste Mac (só Command Line Tools) — `xcodebuild` recusa rodar; requer instalação interativa via App Store com o Apple ID do usuário, não pode ser feito de forma não-interativa. CocoaPods também não está instalado (necessário pra `pod install` antes de abrir o projeto no Xcode)".
...
- [x] Build de release compilável + archive — **resolvido**, confirmado pela submissão de 07/09/2026 (build 1.0(1)). `project.pbxproj` já está preparado pra build 1.0(2) (`CURRENT_PROJECT_VERSION = 2`)
- [ ] TestFlight interno
...
## Estado geral (24/08/2026) — ⚠️ HISTÓRICO, DESATUALIZADO

**Este bloco é um retrato de 24/08/2026, treze dias antes da primeira submissão à App Store (07/09/2026), e não reflete o estado atual (11/09/2026).** Mantido como registro histórico do que faltava naquele momento — não usar como fonte do estado real hoje. Em particular:

- "Build iOS bloqueado por falta de Xcode" (item 1) e "APNs Auth Key" pendente (item 5, parte iOS) estão **resolvidos** — o app já foi submetido em 07/09/2026, o que exige Xcode/build funcionando, e o push nativo iOS está confirmado funcionando em aparelho físico (ver "Bloqueio 2 — iOS", resolvido, e "Notificações push nativas" em `docs/APP_STORE_METADATA.md`).
- Item 3 (conta de revisor) segue **sem confirmação registrada** neste repositório — não presumir que foi resolvido só porque a submissão aconteceu; ver nota em `docs/APP_STORE_METADATA.md` → "Conta de revisor".
- Itens 2 (keystore de produção Android) e 4 (backup off-site) e o Bloqueio 1 (Android/Firebase) não foram reauditados nesta rodada — seguem como estavam em 24/08, sem confirmação de que mudaram.
